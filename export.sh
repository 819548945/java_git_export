...
java(){ 
  ##  createDir "$2/java";
   ## output=$(pwd);
    echo -e "${INFO_COLOR}导出java 文件${RES}";
    cd $1;
    git diff --name-only --diff-filter=M "*.java" | xargs -I {} cp --parents {} $2;
    
  ##  cd $output;
}
class(){
   ## createDir "$2/class";
    echo -e "${INFO_COLOR}导出class 文件${RES}";
    cd $1;
    mvn clean > ${LOG_FILE};
  ...
  if check $1 $2;then
    ...
    if check $1 $2;then
        ...
        if [ $3 = 'java' ];then
            ...
            echo "参数错误类型只允许java或class，当前为$3";
        fi
    fi
else
